...
export default function VoiceChat({ channelName, participants }: VoiceChatProps) {
  const { isMuted, isDeafened, toggleMute, toggleDeafen, leaveVoiceChannel, getAudioLevel, currentUser } =
    useAppContext()
  ...
  return (
    <div className="flex-1 flex flex-col overflow-y-auto bg-[hsl(var(--aura-chat))]">
      <div className="h-14 border-b border-aura-primary/10 flex items-center justify-between px-4">
        <div className="flex items-center">
          <Volume2 size={20} className="mr-2 text-aura-secondary" />
      ...
      <div className="flex-1 p-6 overflow-y-auto scrollable">
        <div className="text-center mb-6">
          ...
          <p className="text-[hsl(var(--aura-text-muted))]">{participants.length} participants</p>
        </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4 mb-6">
          {participants.map((participant) => {
            ...
            return (
              <Card
                key={participant.id}
                className="flex flex-col items-center p-4 bg-aura-bg/50 border-aura-primary/10"
              >
                <div
                  className={`relative ${
                    isSpeaking
                  ...
                  style={{
                    boxShadow: isSpeaking ? `0 0 ${ringSize}px rgba(139, 92, 246, 0.7)` : "none",
                  }}
                >
                ...
                <div className="flex items-center mt-1 text-xs text-[hsl(var(--aura-secondary))]">
                  <div
                    className={`w-2 h-2 rounded-full ${
                      isSpeaking ? "bg-[hsl(var(--aura-success))]" : "bg-[hsl(var(--aura-muted))]"
                    } mr-1`}
                  ></div>
        ...
        <Card className="overflow-hidden bg-aura-bg/50 border-aura-primary/10">
          <CardContent className="p-6 space-y-4">
            <div className="flex items-center justify-between">
              <span className="text-sm text-[hsl(var(--aura-text-muted))]">Volume</span>
            ...
            <Tooltip>
              <TooltipTrigger asChild>
                <Button
                  ...
                  className={
                    isDeafened ? "bg-aura-danger hover:bg-aura-danger/90" : "bg-aura-muted/60 hover:bg-aura-muted/80"
                  }
                >
